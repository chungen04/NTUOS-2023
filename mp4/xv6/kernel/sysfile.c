...
>>>>>>> 9029cc1fda289fb3b7e5ebb059a3e350e6fe90da
  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
    ...
    if(unlock_symlink){
      iunlockput(ip); // link target.
      iunlockput(origin_ip); // link source
    }else{
      iunlockput(ip);
    }
    end_op();
  ...
  if(unlock_symlink){
    iunlock(ip); // link target.
    iunlock(origin_ip); // link source
  }else{
    iunlock(ip);
  }
  end_op();
